...
usuarioController.usuarioCreatet= async (req,res,next)=>{
    const {nombre,apellido,email,password ,password_conf,img,telefono,sexo,edad}=(req.body);
    console.log({nombre,apellido,email,password ,password_conf,img,telefono,sexo,edad});
    ...
    if (errors.length > 0) {
        ...
        if(emailUsuario) {res.send({message:"usuario ya registrado",'success':false});
            ...
            const usuario =new usuarioModel({nombre,apellido,email,password:bcrypt.hashSync(password,10),img,telefono,sexo,edad});
            await usuario.save();   
            console.log(usuario);
            res.json(usuario);
        } 
